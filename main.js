// Baraka Market - FullStack API Integration (Final Fix)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (target.closest('.close-cart, .close-wishlist, .close-modal, .close-checkout') || target.classList.contains('cart-overlay')) closeAll();
    });
...
function updateCheckoutUI(step) {
    ...
    if (step === 3) {
        document.getElementById('summary-count').innerText = `${(JSON.parse(localStorage.getItem('cart')) || []).length} ta`;
...
function toggleWishlist(id) {
    ...
    localStorage.setItem('wishlist', JSON.stringify(wishlist)); renderProducts(products); updateWishlistUI();
}
